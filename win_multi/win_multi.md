...
Let's make multiple image processing games on windows and a ROS node which can communicate with each other via TCP/IP.
...
## :o:Practice1(Single image processing program and a ros node)
...
- Save the following two files into `C:\oit\py22_ipbl\code` on Windows.
  ...
  - [show_hand_game_on_win.py](./win/show_hand_game_on_win.py)
    - This program runs an image processing game while communicating with the ROS.
  - [start_single_on_win.py](./win/start_single_on_win.py)
...
- You can test only this image processing game without communication with the ROS by using the following command.
  - When executed, the command calls `demo` function in the `show_hand_game_on_win.py`, and runs demo of `show hand game` without communication with the ROS.
  - We strongly recommend to implement the `demo` function like this even if you create new original image processing games.

```sh
C:\\...\code> python show_hand_game_on_win.py
```

- This program's `start_game` function is called from `start_single_on_win.py` when the received ROS message is "[shg]start".
...
#### start_single_on_win.py
- After confirming that `show_hand_game_on_win.py` runs normally at alone, Confirm that the game `show_hand_game` called from `start_single_on_win.py` runs normally while communicating with the ROS, as following command.
  - Now, `start_single_on_win.py` waits in the infinite loop for a message from the ROS with `while True:`, and exits the infinite loop when the message "[all]end" is received from the ROS.

```sh
C:\\...\code> python start_single_on_win.py
```

- If you want to add a new image processing program that communicates with ROS (e.g. `show_hand_game_on_win.py`), you have to edit the following part of `start_single_on_win.py`.
  ...
  - `shg.start_game(topic_name_from_win, ros_bridge_tcp)` calls `start_game` function of the image processing program (i.e. `show_hand_game_on_win.py`) that communicates with the ROS.
    - `import show_hand_game_on_win as shg` is also required.
...
- Save the following two files into `~/catkin_ws/src/oit_pbl_ros_samples/scripts/` on ROS.
  ...
  - [show_hand_game_on_ros.py](./ros/show_hand_game_on_ros.py)
    - This is the ROS program which communicates with the image processing game (i.e. `show_hand_game_on_win.py`) on the Windows.
  - [start_on_ros_single.py](./ros/start_on_ros_single.py)
    - This program runs on the ROS container, and then calls `show_hand_game_on_ros.py` which send the message to the image processing program on Windows.

#### show_hand_game_on_ros.py
- `play_show_hand_game` function of this program communicates with `show_hand_game_on_win.py` on Windows.
- Although this program is generally called from `start_on_ros_signal.py` as module, you can also run this program alone.
  ...
  - Allow the parmission for execution and run the launch program.
    ```sh
    $ chmod u+x show_hand_game_on_ros.py
...
```sh
$ rosrun oit_pbl_ros_samples show_hand_game_on_ros.py
```

- The communication process between the ROS and Windows in `show_hand_game_on_ros.py` is as follows. Please read and understand the comments carefully.
...
- After confirming that `show_hand_game_on_ros.py` runs normally, Confirm that both `start_on_ros_single.py` and `show_hand_game_on_ros.py` runs normally, as following command.
...
- If you want to add a new ROS program (e.g. `show_hand_game_on_ros.py`) that communicates with Windows side, you have to edit the following part of `start_on_ros_single.py` as follows.
  - `result = shgr.play_show_hand_game()` calls `show_hand_game_on_ros.py` as module.
    - `import show_hand_game_on_ros as shgr` must be added.
...
### Reference
- The following figure shows the flow of programs when you run `show_hand_game_on_ros.py` alone on the ROS after running `start_single_on_win.py` on Windows.
<image src="./communication_win-ros.png">

## Exercise (add another service)
### Windows side
- In Practice, you have implemented the communication process between `show_hand_game_on_win.py` and ROS (`show_hand_game_on_ros.py`).
- In this Exercise, you will add the bright and dark game.
- First, download `bright_dark_game_on_win.py` and save it in your `code` folder (on windows). 
  - If you want to download the file, click the link and then click the `Raw` button.
  - [bright_dark_game_on_win.py](./win/bright_dark_game_on_win.py)
    - An image processing program that communicates with ROS
  - This program is a game that brightens or darkens the camera image according to the "bright" and "dark" commands given by the ROS. You can adjust the brightness by pointing the camera at the light or covering the camera with your hand. If you can't switch between bright and dark as you want, try changing the parameters in the `judge_game` function in `bright_dark_game_on_win.py`.


- Next, run `bright_dark_game_on_win.py` and check if it works correctly.
  - In some cases, it may not start properly. Then, try restarting the Windows Terminal.

```sh
C:\\...\code> python bright_dark_game_on_win.py
```
- After confirming that the above program are executed correctly, copy `start_single_on_win.py`, rename it to `start_multi_on_win.py` and save it in the `code` folder (on Windows).
- Next, add the following if statement to the next line of `shg.start_game(topic_name_from_win, ros_bridge_tcp)` in `start_multi_on_win.py` and add the import statement `import bright_dark_game_on_win as bdg` at the beginning of the program.
  - The following code calls the `start_game` function in `bright_dark_game_on_win.py`.
...
- Download `bright_dark_game_on_ros.py` and save it in the folder `~/catkin_ws/src/oit_pbl_ros_samples/scripts/` (in the ROS container). 
  - [bright_dark_game_on_ros.py](./ros/bright_dark_game_on_ros.py)
    - This is a ROS-side program to communicate with `bright_dark_game_on_win.py`.
...
- After you have correctly placed `bright_dark_game_on_ros.py` in the ROS container, run the following command in the ROS container to see if it works correctly.
  - If you did not run the launch program, run the launch program on other terminal `roslaunch oit_stage_ros navigation.launch`


```sh
$ chmod u+x bright_dark_game_on_ros.py
```

- Next, start the following program on Windows side.

```sh
C:\\...\code> python start_multi_on_win.py
```

- Next, if you run the following command in the ROS container, the `demo()` function of the `bright_dark_game_on_ros.py` will be called through ROS, and communication will be established between ROS side and Windows side.
  - If you run only `bright_dark_game_on_ros.py`, `start_multi_on_win.py` will not be terminated because the termination process is not sent to Windows side from ROS side. If you want to exit `start_multi_on_win.py`, send `Ctr+C`.

```sh
$ rosrun oit_pbl_ros_samples bright_dark_game_on_ros.py 
``` 

- Once you have verified that `bright_dark_game_on_ros.py` runs correctly, copy `start_on_ros_single.py`, rename it to `start_on_ros_multi.py`, and save it in the `~/catkin_ws/src/oit_ pbl_ros_samples/scripts/` folder (in the ROS container). 
- Then, add a procedure invoking the  `bright_dark_game_on_ros.py` in `start_on_ros_multi.py`.
  - Add the following code snippet before `end_game()` in `process()` of `start_on_ros_multi.py`.
  - `import bright_dark_game_on_ros as bdg` is also required.
...
```python
    ...
    rospy.sleep(5)
```
- This code fragment shows the process of executing `play_bright_dark_game()` in `bright_dark_game_on_ros.py` and receiving the result.
...
### ROS side
- Download `move_robot_on_ros.py` and save it in the folder `~/catkin_ws/src/oit_pbl_ros_samples/scripts/` (in the ROS congainer). 
  - [move_robot_on_ros.py](./ros/move_robot_on_ros.py)
...
- After you have correctly placed `move_robot_on_ros.py` in the ROS container, run the following command in the ROS container to see if it works correctly.
- Run the following command in the ROS container
  - If you did not run the launch program, run the launch program on other terminal `roslaunch oit_stage_ros navigation.launch`

```sh
$ chmod u+x move_robot_on_ros.py
```

- Next, run the following command in the ROS container, and the `demo()` function of `move_robot_on_ros.py` will be called through ROS, and the robot will move to the specified location on the stage simulator.

```sh
$ rosrun oit_pbl_ros_samples move_robot_on_ros.py 
``` 

- After checking that `move_robot_on_ros.py` works correctly, add a procedure to `start_on_ros_multi.py` that calls `move_robot_on_ros.py`.
  - Add the following code snippet before the line `print("---bdg---")` in the `process()` function of `start_on_ros_multi.py`. This will call `process()` in `move_robot_on_ros.py` to move the robot.
  - `import move_robot_on_ros as mrobot` is also required.
